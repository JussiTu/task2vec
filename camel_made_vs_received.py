"""
Two side-by-side CAMEL network plots:
...
with open('.cache/apache_learning_signals.json') as f:
    data = json.load(f)
trajectories = {}
for eng in data.get('CAMEL', {}).get('engineers', []):
    rho = eng.get('rho', 0)
...
fig.suptitle(
    'Apache Camel — Communication Network\n'
...
out = '.cache/camel_made_vs_received.png'
