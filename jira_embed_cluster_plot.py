# jira_embed_cluster_plot.py
# Embed Jira issues, cluster, project to 2D, plot,
# then label clusters automatically using an LLM.
#
# Run:
#   python jira_embed_cluster_plot.py --input jira_100.jsonl --limit 100
#
...
def main():
    ...
    plt.scatter(X2[:, 0], X2[:, 1], s=36)
    plt.title("Jira tickets: 2D projection (embeddings)")
    ...
    for i in range(min(12, len(keys))):
        plt.text(X2[i, 0], X2[i, 1], keys[i], fontsize=8)
