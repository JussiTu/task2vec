"""
Two side-by-side HADOOP network plots:
...
with open('.cache/apache_learning_signals.json') as f:
    data = json.load(f)
trajectories = {}
for eng in data.get('HADOOP', {}).get('engineers', []):
    rho = eng.get('rho', 0)
...
fig.suptitle(
    'Apache Hadoop — Communication Network\n'
...
out = '.cache/hadoop_made_vs_received.png'
